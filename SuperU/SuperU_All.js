...
async function search_produits(page, df_rayons, nom_drive, department, city, today,year, month) {
    ...
    fs.writeFileSync(`${newpath}/SuperU_produits.csv`, df_produits.map(obj => Object.values(obj).join(',')).join('\n'));
...
async function dataPrixClean(df_produits, nom_drive, department, city, nom_driveUrl, today, year, month) {
    ...
    const filePath = `../../Data_base/${year}/${month}/${department}/${city}/${nom_drive}/${today}/SuperU_produits.csv`;
...
async function scrapAll(today, year,month,page) {
    ...
    try {
        df_drivesScraped = fs.readFileSync(`../../Data_base/${year}/${month}/drivesScraped/list_drives_SuperUScraped.csv`, 'utf-8')
            .split('\n')
            .map(line => line.split(','))
            .map(arr => ({
                id: arr[0],
    ...
    for (let i = 0; i < df.length; i++) {
        ...
        if (indexLastDriveScraped < indexActual) {
            ...
            if (nom_drive.includes("Hyper U")) {
                ...
                fs.writeFileSync(`../../Data_base/${year}/${month}/drivesScraped/list_drives_SuperUScraped.csv`, df_drivesScraped.map(obj => Object.values(obj).join(',')).join('\n'));
    ...
    while (true) {
        try {
            ...
            await scrapAll(today, year,month,page).then(() => {
                ...
                const filePath = `../../Data_base/${year}/${month}/drivesScraped/list_drives_SuperUScraped.csv`;
